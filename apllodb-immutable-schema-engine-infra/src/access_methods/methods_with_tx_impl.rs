...
impl<'sess> MethodsWithTxImpl<'sess> {
    ...
    fn database_name<'s>(&self, session: &'s SessionWithTx) -> &'s DatabaseName {
        session.get_db_name()
    }

    fn sqlite_tx(&self, session: &SessionWithTx) -> ApllodbResult<&SqliteTx> {
        self.tx_repo.get(session.get_id())
    }
